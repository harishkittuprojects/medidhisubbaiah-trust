// Universal Icon mapper supporting Lucide icons for React
const { createElement } = React;

// Fallback SVG Icons if lucide-react is loaded via window.lucide or SVG
export function Icon({ name, className = "w-5 h-5", size = 20, color = "currentColor" }) {
  // Check if lucide object is on window
  if (window.lucide && window.lucide.icons) {
    const iconKey = name ? name.toLowerCase().replace(/[-_ ]/g, '') : 'heart';
    // Find matching icon
    const matchedKey = Object.keys(window.lucide.icons).find(
      k => k.toLowerCase() === iconKey || k.toLowerCase().includes(iconKey)
    );

    if (matchedKey && window.lucide.icons[matchedKey]) {
      const svgString = window.lucide.icons[matchedKey].toSvg({
        class: className,
        width: size,
        height: size,
        color: color,
  ...
  // Built-in SVG fallbacks for core icons
  const iconMap = {
    scissors: (
