...
const initialUpiDetails = {
  upiId: "medidhisubbaiah@sbi",
  payeeName: "Medidhisubbaiah Trust",
  qrCodeUrl: "",
  bankName: "State Bank of India",
  accountHolder: "Medidhisubbaiah Trust",
  accountNumber: "3892019284729",
  ifsc: "SBIN0004521",
  branch: "Main Community Branch, Nizampet",
  instructions: "Scan via Google Pay, PhonePe, Paytm, BHIM, or any UPI App for instant 80G tax-exempt donation."
};
...
const TrustProvider = ({ children }) => {
  ...
  const [upiDetails, setUpiDetails] = useState(() => {
    try {
      const saved = localStorage.getItem('mst_upi_details');
      return saved ? JSON.parse(saved) : initialUpiDetails;
    } catch {
      return initialUpiDetails;
    }
  });
  ...
  const updateUpiDetails = (newUpi) => {
    setUpiDetails(newUpi);
    localStorage.setItem('mst_upi_details', JSON.stringify(newUpi));
    showToast('UPI & Bank Payment details updated successfully!', 'success');
  };
  ...
  useEffect(() => {
    localStorage.setItem('mst_upi_details', JSON.stringify(upiDetails));
  }, [upiDetails]);
  ...
  const requestPasswordReset = async (email) => {
    ...
    try {
      ...
      if (supabaseClient) {
        try {
          await supabaseClient.from('admin_password_resets').insert([{
            id: 'rst_' + Date.now(),
            email: targetEmail,
            token: mockToken,
      ...
      return {
        success: true,
        message: `Password reset request created for ${targetEmail}. Please check your email inbox.`,
        resetToken: mockToken
      };
    }
  };

  const resetAdminPassword = async (token, newPassword, email) => {
    const targetEmail = (email || 'medidhisubbaiahtrustorg@gmail.com').toLowerCase().trim();
    try {
      ...
      console.warn('API reset password err:', err);
    }

    localStorage.setItem('mst_admin_custom_password', newPassword);
    if (supabaseClient) {
      try {
        await supabaseClient.from('admin_auth').upsert({
          id: 'primary_admin',
          email: targetEmail,
        ...
        console.warn('Supabase password save notice:', sbErr);
      }
    }
  ...
  return (
    <TrustContext.Provider value={{
      ...
      trustInfo: {
        ...
        accountDetails: {
          bank: upiDetails.bankName,
          accountNumber: upiDetails.accountNumber,
          ifsc: upiDetails.ifsc,
          branch: upiDetails.branch
        }
      },
      upiDetails,
      updateUpiDetails,
...
// --- LOGIN PAGE ---
const LoginPage = () => {
  const { loginAdmin, requestPasswordReset, trustInfo, navigate } = useTrust();
  ...
  const handleFillDemo = () => {
    setEmail('medidhisubbaiahtrustorg@gmail.com');
  ...
  const handleSendResetEmail = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setForgotMsg(res.message || `Password reset instructions sent to ${forgotEmail}`);
        if (res.resetToken) setSentToken(res.resetToken);
      } else {
        setErrorMessage(res.error || 'Failed to send reset link.');
      }
    } catch (err) {
      setErrorMessage(err.message || 'An unexpected error occurred while sending reset email.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100 w-full max-w-full">
      ...
      <div className="flex-1 flex items-center justify-center py-10 px-4">
        <div className="max-w-md w-full space-y-5 bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-xl" data-aos="zoom-in">
          ...
          <div className="text-center space-y-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                {viewMode === 'login'
                  ? 'Sign in to manage Events, Media Gallery, Press News & Services'
                  : 'Receive a password reset link and security OTP via Gmail SMTP'}
              </p>
            </div>
          </div>
          ...
          {viewMode === 'login' && (
            <>
              {errorMessage && (
                <div className="bg-red-50 border border-red-300 text-red-700 px-3 py-2 rounded-xl text-xs flex items-center space-x-2">
              ...
              <form onSubmit={handleLogin} className="space-y-3.5 text-xs sm:text-sm">
                <div>
                  <label className="font-bold font-heading block text-xs text-slate-700 mb-1">Administrator Email</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. medidhisubbaiahtrustorg@gmail.com"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className="w-full p-2.5 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"
                  />
                </div>
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="font-bold font-heading block text-xs text-slate-700">Password</label>
                    <button
                      type="button"
                      onClick={() => {
                        setViewMode('forgot');
                      ...
                      className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700 hover:underline flex items-center space-x-1"
                    >
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full p-2.5 pr-9 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"
                    />
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold font-heading py-3 rounded-xl shadow-md transition flex items-center justify-center space-x-2"
                >
                  {isLoading ? (
                ...
                <div className="pt-2 text-center border-t border-slate-100">
                  <button
                    ...
                    className="w-full py-2.5 px-3 border border-emerald-300 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-bold rounded-xl text-xs flex items-center justify-center space-x-2 transition"
                  >
                    <Icon name="mail" size={14} className="text-emerald-700" />
                    <span>Forgot Password? Send Reset Link to Email</span>
                  </button>
                </div>
              </form>
            </>
              ...
              {!forgotSuccess ? (
                <>
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-900">
                    <p className="font-bold flex items-center space-x-1.5 mb-1">
                      ...
                      <span>SMTP Email Password Reset</span>
                    </p>
                    <p className="text-[11px] text-amber-800 leading-relaxed">
                      Enter your official administrator email. A secure password reset link and 6-digit security code will be sent to your Gmail inbox.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleSendResetEmail} className="space-y-3 text-xs sm:text-sm">
                    <div>
                      <label className="font-bold font-heading block text-xs text-slate-700 mb-1">Administrator Email Address</label>
                      <input
                        type="email"
                        required
                        ...
                        className="w-full p-2.5 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-mono text-xs"
                      />
                    </div>
                    ...
                    <button
                      type="submit"
                      disabled={isLoading}
                      className="w-full bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold font-heading py-2.5 rounded-xl shadow-md transition flex items-center justify-center space-x-2"
                    >
                      {isLoading ? (
                ...
                <div className="space-y-4 text-center">
                  ...
                  <div className="space-y-1">
                    <h3 className="font-extrabold text-sm text-slate-900 font-heading">Reset Email Sent Successfully!</h3>
                    <p className="text-xs text-slate-600 leading-relaxed px-2">
                      We have dispatched a reset link and 6-digit OTP code to <strong className="text-emerald-700">{forgotEmail}</strong>.
                    </p>
                  </div>

                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-600 text-left space-y-1">
                    <span className="font-bold text-slate-800 block text-[11px]">Next steps:</span>
                    <p className="text-[11px]">1. Check your inbox or spam folder for an email from <strong>Medidhisubbaiah Trust</strong>.</p>
                    <p className="text-[11px]">2. Click the <strong>Reset Admin Password</strong> link in the email, or click below to proceed directly.</p>
                  </div>

                  <button
                    type="button"
                    onClick={() => {
                      const tokenParam = sentToken ? `?token=${sentToken}&email=${encodeURIComponent(forgotEmail)}` : `?email=${encodeURIComponent(forgotEmail)}`;
                      navigate(`reset-password${tokenParam}`);
                    }}
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold font-heading py-2.5 rounded-xl shadow-md transition text-xs flex items-center justify-center space-x-1.5"
                  >
...
const ResetPasswordPage = () => {
  ...
  // Extract token & email from query params / hash
    ...
    <div className="min-h-screen flex flex-col bg-slate-100 w-full max-w-full">
      ...
      <div className="bg-slate-950 text-white px-4 sm:px-8 py-3.5 flex justify-between items-center border-b border-slate-800 shadow-md">
        <div className="flex items-center space-x-3 cursor-pointer" onClick={() => navigate('home')}>
          ...
          <div>
            <span className="font-bold text-sm text-white font-heading">Medidhisubbaiah <span className="text-emerald-400">Trust</span></span>
            <span className="text-[10px] text-slate-400 block font-heading">Security Management</span>
          </div>
        </div>
        <button
      ...
      <div className="flex-1 flex items-center justify-center py-10 px-4">
        <div className="max-w-md w-full space-y-5 bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-xl" data-aos="zoom-in">
          ...
          <div className="text-center space-y-2">
            <div className="w-16 h-16 rounded-full p-1 bg-gradient-to-tr from-emerald-600 via-teal-500 to-amber-400 shadow-lg mx-auto flex items-center justify-center">
              <Icon name="lock" size={28} className="text-white" />
            </div>
            <div>
              <h2 className="text-xl sm:text-2xl font-black font-heading text-slate-900">Set New Admin Password</h2>
              <p className="text-xs text-slate-500">Enter and confirm your new administrator credentials</p>
            </div>
          </div>

          {!isSuccess ? (
            <form onSubmit={handleReset} className="space-y-3.5 text-xs sm:text-sm">
              {errorMessage && (
                <div className="bg-red-50 border border-red-300 text-red-700 px-3 py-2 rounded-xl text-xs flex items-center space-x-2">
                  <Icon name="x" size={14} className="shrink-0" />
                  <span>{errorMessage}</span>
              ...
              <div>
                ...
                <input
                  ...
                  readOnly
                  value={email}
                  className="w-full p-2.5 border border-slate-300 rounded-xl bg-slate-100 text-slate-600 font-mono text-xs cursor-not-allowed"
                />
              </div>

              <div>
                <label className="font-bold font-heading block text-xs text-slate-700 mb-1">New Password</label>
                <div className="relative">
                  <input
                    type={showPassword ? 'text' : 'password'}
                    required
                    ...
                    className="w-full p-2.5 pr-9 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2.5 top-2.5 text-slate-400 hover:text-slate-600">
                    <Icon name={showPassword ? 'eyeoff' : 'eye'} size={15} />
                  </button>
              ...
              <div>
                <label className="font-bold font-heading block text-xs text-slate-700 mb-1">Confirm New Password</label>
                <input
                  type={showPassword ? 'text' : 'password'}
                  required
                  minLength={6}
                  ...
                  className="w-full p-2.5 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
            ...
            <div className="space-y-4 text-center py-2">
              ...
              <div className="space-y-1">
                <h3 className="font-extrabold text-base text-slate-900 font-heading">Password Changed Successfully!</h3>
                <p className="text-xs text-slate-600 leading-relaxed">
                  Your administrator password has been updated in the security system.
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={() => navigate('login')}
                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold font-heading py-3 rounded-xl shadow-lg transition text-xs flex items-center justify-center space-x-2"
              >
...
const {
  ...
  // UPI & Bank Form State
  const [upiForm, setUpiForm] = useState(() => upiDetails || initialUpiDetails);
      ...
      <div className="md:hidden bg-slate-950 text-white p-3 border-b border-slate-800 space-y-2.5 shrink-0">
        ...
        <div className="flex space-x-1.5 overflow-x-auto scrollbar-none pb-1">
          {[
            ...
            { id: 'upi', label: 'UPI & Payments', count: null, icon: 'creditcard' },
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-1">
            ...
            {[
              ...
              { id: 'upi', label: 'UPI & Bank Payments', count: null, icon: 'creditcard' },
      ...
      {activeTab === 'upi' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className="bg-amber-100 text-amber-800 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full font-heading">
                  Instant Public Sync
                </span>
              </div>
              <h2 className="text-xl font-black font-heading text-slate-900 mt-1">UPI &amp; Bank Account Configuration</h2>
              <p className="text-xs text-slate-500">
                Update the official Trust UPI ID, QR code, and bank account details displayed to donors across the website.
              </p>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <button
                type="button"
                onClick={() => setIsDonateModalOpen(true)}
                className="bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-300 font-bold font-heading px-3.5 py-2 rounded-xl text-xs flex items-center space-x-1.5 transition"
              >
                <Icon name="eye" size={13} />
                <span>Test Live Donate Modal</span>
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            {/* Left: Dynamic QR Code Card Preview */}
            <div className="md:col-span-5 bg-gradient-to-br from-emerald-900 via-teal-900 to-slate-950 text-white p-6 rounded-3xl shadow-xl flex flex-col items-center justify-between text-center space-y-4 border border-emerald-700/50">
              <div className="space-y-1">
                <span className="bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full font-heading">
                  Live Donor QR Preview
                </span>
                <h3 className="text-lg font-black font-heading">{upiForm.payeeName || 'Medidhisubbaiah Trust'}</h3>
                <p className="text-xs font-mono text-emerald-300 font-bold select-all">{upiForm.upiId || 'medidhisubbaiah@sbi'}</p>
              </div>

              {/* Dynamic QR Code Box */}
              <div className="bg-white p-3.5 rounded-2xl shadow-2xl border-4 border-emerald-400/40 relative group">
                <img
                  src={
                    upiForm.qrCodeUrl && upiForm.qrCodeUrl.trim()
                      ? upiForm.qrCodeUrl
                      : `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(`upi://pay?pa=${upiForm.upiId || 'medidhisubbaiah@sbi'}&pn=${encodeURIComponent(upiForm.payeeName || 'Medidhisubbaiah Trust')}&cu=INR`)}`
                  }
                  alt="Live UPI QR Code"
                  className="w-48 h-48 sm:w-52 sm:h-52 object-contain rounded-xl"
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(`upi://pay?pa=${upiForm.upiId || 'medidhisubbaiah@sbi'}&pn=MedidhisubbaiahTrust&cu=INR`)}`;
                  }}
                />
                <div className="mt-2 flex items-center justify-center space-x-1.5 text-slate-800 text-[11px] font-black font-heading">
                  <Icon name="qr" size={13} className="text-emerald-600" />
                  <span>Scan with GPay / PhonePe / Paytm</span>
                </div>
              </div>

              <div className="text-[11px] text-emerald-200/80 leading-relaxed max-w-xs">
                {upiForm.instructions || 'Scan with any UPI app to make a direct 80G tax-exempt donation.'}
              </div>
            </div>

            {/* Right: Bank Details Card Preview */}
            <div className="md:col-span-7 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between space-y-4">
              <div className="space-y-3">
                <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                  <div className="flex items-center space-x-2.5">
                    ...
                    <div>
                      <h4 className="font-extrabold text-sm text-slate-900 font-heading">Official Bank Account Card</h4>
                      <span className="text-[10px] text-slate-500 block font-heading">NEFT / RTGS / IMPS Direct Transfer</span>
                    </div>
                  </div>
                  <span className="bg-emerald-100 text-emerald-800 text-[10px] font-bold px-2.5 py-1 rounded-full font-heading">
                    Verified Active
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <span className="text-[10px] uppercase font-bold text-slate-400 font-heading block">Bank Name</span>
                    <span className="font-extrabold text-slate-900 font-heading block mt-0.5">{upiForm.bankName || 'State Bank of India'}</span>
                  </div>

                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <span className="text-[10px] uppercase font-bold text-slate-400 font-heading block">Account Holder</span>
                    <span className="font-extrabold text-slate-900 font-heading block mt-0.5">{upiForm.accountHolder || 'Medidhisubbaiah Trust'}</span>
                  </div>

                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] uppercase font-bold text-slate-400 font-heading">Account Number</span>
                      <button
                        type="button"
                        onClick={() => {
                          navigator.clipboard.writeText(upiForm.accountNumber);
                    ...
                    <span className="font-mono font-bold text-slate-900 block mt-0.5">{upiForm.accountNumber || '3892019284729'}</span>
                  </div>

                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] uppercase font-bold text-slate-400 font-heading">IFSC Code</span>
                      <button
                        type="button"
                        onClick={() => {
                          navigator.clipboard.writeText(upiForm.ifsc);
                    ...
                    <span className="font-mono font-bold text-slate-900 block mt-0.5">{upiForm.ifsc || 'SBIN0004521'}</span>
                  </div>
                </div>

                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-xs">
                  <span className="text-[10px] uppercase font-bold text-slate-400 font-heading block">Branch &amp; City</span>
                  <span className="font-semibold text-slate-800 block mt-0.5">{upiForm.branch || 'Main Community Branch, Nizampet, Hyderabad'}</span>
                </div>
              </div>

              <div className="pt-2 flex items-center justify-between border-t border-slate-100 text-xs">
                <span className="text-slate-500 text-[11px]">80G Tax Exemption receipts generated for all verified entries</span>
                <span className="text-emerald-700 font-bold font-heading text-xs">100% Tax Deductible</span>
              </div>
            </div>
          </div>

          {/* Edit Form */}
          <form onSubmit={handleUpiSubmit} className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <h3 className="text-base font-black font-heading text-slate-900 flex items-center space-x-2">
                <Icon name="edit" size={16} className="text-emerald-600" />
                <span>Edit UPI &amp; Bank Settings</span>
              </h3>
              <button
                type="button"
                onClick={() => {
                  if (confirm('Reset payment details to official default SBI account?')) {
                    setUpiForm(initialUpiDetails);
                    updateUpiDetails(initialUpiDetails);
                  }
                }}
            ...
            {/* Section 1: UPI Gateway */}
            <div className="space-y-4">
              <h4 className="text-xs uppercase font-extrabold text-emerald-800 tracking-wider font-heading">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. medidhisubbaiah@sbi or 9876543210@paytm"
                    value={upiForm.upiId || ''}
                  ...
                  />
                  <span className="text-[10px] text-slate-400 block mt-0.5">Donors can copy or scan this ID</span>
                </div>

                <div>
                  <label className="block text-xs font-bold font-heading text-slate-700 mb-1">
                    Payee / Trust Display Name *
                  </label>
                  <input
                    type="text"
                    required
                    placeholder="Medidhisubbaiah Trust"
                  ...
                  />
                  <span className="text-[10px] text-slate-400 block mt-0.5">Name displayed in UPI apps upon scanning</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    placeholder="Leave empty to auto-generate dynamic QR, or paste Cloudinary URL"
                    value={upiForm.qrCodeUrl || ''}
                    onChange={(e) => setUpiForm({ ...upiForm, qrCodeUrl: e.target.value.trim() })}
                    className="flex-1 p-2.5 border border-slate-300 rounded-xl font-mono text-xs focus:ring-2 focus:ring-emerald-500"
                  />
                  <label className="cursor-pointer bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 text-xs font-bold font-heading px-3.5 py-2.5 rounded-xl shadow-sm transition flex items-center space-x-1 shrink-0">
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          const res = await uploadToCloudinary(file);
                          setUpiForm({ ...upiForm, qrCodeUrl: res.url });
                          showToast('Custom QR uploaded successfully!', 'success');
                        } catch (err) {
                          const reader = new FileReader();
                          reader.onload = () => {
                            setUpiForm({ ...upiForm, qrCodeUrl: reader.result });
                            showToast('QR loaded from file!', 'info');
                          };
                          reader.readAsDataURL(file);
                        }
                      }}
                ...
                <span className="text-[10px] text-slate-400 block mt-0.5">If left empty, a crisp QR code matching your UPI ID is dynamically generated automatically.</span>
              </div>
            </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-bold font-heading text-slate-700 mb-1">Bank Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-bold font-heading text-slate-700 mb-1">Account Holder Name *</label>
                  <input
                    type="text"
                    required
                    placeholder="Medidhisubbaiah Trust"
                    value={upiForm.accountHolder || ''}
                    onChange={(e) => setUpiForm({ ...upiForm, accountHolder: e.target.value })}
                    className="w-full p-2.5 border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold font-heading text-slate-700 mb-1">Account Number *</label>
                  <input
                    type="text"
                    required
                    placeholder="3892019284729"
                    value={upiForm.accountNumber || ''}
                    onChange={(e) => setUpiForm({ ...upiForm, accountNumber: e.target.value.trim() })}
                    className="w-full p-2.5 border border-slate-300 rounded-xl font-mono text-xs focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold font-heading text-slate-700 mb-1">IFSC Code *</label>
                  <input
                    type="text"
                    required
                    placeholder="SBIN0004521"
                    value={upiForm.ifsc || ''}
                    onChange={(e) => setUpiForm({ ...upiForm, ifsc: e.target.value.toUpperCase().trim() })}
                    className="w-full p-2.5 border border-slate-300 rounded-xl uppercase font-mono text-xs focus:ring-2 focus:ring-emerald-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold font-heading text-slate-700 mb-1">Branch Name &amp; Location</label>
                  <input
                    type="text"
                    placeholder="Main Community Branch, Nizampet, Hyderabad"
                    value={upiForm.branch || ''}
                    onChange={(e) => setUpiForm({ ...upiForm, branch: e.target.value })}
                    className="w-full p-2.5 border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold font-heading text-slate-700 mb-1">
                  Donation Instructions &amp; Tax Info
                </label>
                <textarea
                  rows={2}
                  placeholder="Scan via Google Pay, PhonePe, Paytm, BHIM, or any UPI App for instant 80G tax-exempt donation."
            ...
            <div className="pt-2 flex items-center justify-end space-x-3">
              <button
                type="submit"
                className="bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold font-heading px-6 py-3 rounded-xl shadow-lg transition text-xs flex items-center space-x-2"
              >
                <Icon name="check" size={16} />
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-fadeIn">
      <div className="bg-white rounded-3xl max-w-lg w-full p-5 sm:p-7 space-y-4 shadow-2xl border my-8 max-h-[92vh] overflow-y-auto">
        ...
        {/* Official UPI Details Card with Dynamic QR */}
        <div className="bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-50 p-4 rounded-2xl border border-emerald-200 flex flex-col sm:flex-row items-center gap-3.5">
          <div className="bg-white p-1.5 rounded-xl border border-emerald-200 shadow-sm shrink-0">
            <img
              src={
                upiDetails?.qrCodeUrl && upiDetails.qrCodeUrl.trim()
                  ? upiDetails.qrCodeUrl
                  : `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(`upi://pay?pa=${trustInfo.upiId}&pn=${encodeURIComponent(upiDetails?.payeeName || 'Medidhisubbaiah Trust')}&cu=INR`)}`
              }
          ...
          <div className="min-w-0 flex-1 space-y-1 text-center sm:text-left">
            <span className="text-[10px] font-bold text-emerald-800 uppercase block font-heading">
              Official Trust UPI ID:
            </span>
            <span className="font-mono text-sm font-extrabold text-slate-900 select-all block break-all">
              {trustInfo.upiId}
            </span>
            <span className="text-[10px] text-slate-500 block">
              {trustInfo.accountDetails?.bank || 'State Bank of India'} • A/C: {trustInfo.accountDetails?.accountNumber} • IFSC: {trustInfo.accountDetails?.ifsc}
            </span>
            <button
              type="button"
              onClick={() => {
                navigator.clipboard.writeText(trustInfo.upiId);
                showToast('UPI ID copied: ' + trustInfo.upiId, 'success');
              }}
              className="mt-1 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold font-heading px-3 py-1.5 rounded-lg shadow inline-flex items-center space-x-1 transition"
            >
              <Icon name="copy" size={12} />
...
const App = () => {
  ...
  const isResetPassword = cleanRoute.startsWith('reset-password') || cleanRoute.startsWith('forgot-password');
  const isAdminView = cleanRoute === 'admin' || cleanRoute === 'login' || cleanRoute === 'admin-login' || cleanRoute === 'dashboard' || isResetPassword;
